...
export function App(): JSX.Element {
  ...
  return (
    <>
      ...
      <div style={{ width: 240, flex: 'none', borderRight: `1px solid ${C.line}`, padding: 16, overflow: 'auto', background: C.panel }}>
        ...
        {comps.map((c) => (
          <div
            key={c.id}
            onClick={() => select(c.id)}
            style={{ padding: '10px 12px', borderRadius: 8, cursor: 'pointer', marginBottom: 4, background: c.id === selectedId ? C.line : 'transparent' }}
          >
      ...
      {/* props + render */}
      <div style={{ width: 340, flex: 'none', borderLeft: `1px solid ${C.line}`, padding: 16, display: 'flex', flexDirection: 'column', background: C.panel }}>
        <div style={{ fontWeight: 600, marginBottom: 10 }}>Props</div>
        <textarea
          value={propsText}
          onChange={(e) => setPropsText(e.target.value)}
          spellCheck={false}
          style={{ flex: 1, background: '#141417', color: C.text, border: `1px solid ${propsObj ? C.line : C.accent}`, borderRadius: 8, padding: 12, font: '12px ui-monospace, monospace', resize: 'none', outline: 'none' }}
        />
        {!propsObj && <div style={{ color: C.accent, fontSize: 12, marginTop: 6 }}>invalid JSON</div>}
        <button
          onClick={render}
          disabled={rendering || !propsObj}
          style={{ marginTop: 12, padding: 13, borderRadius: 8, border: 'none', background: rendering ? C.line : C.accent, color: '#fff', fontWeight: 600, cursor: rendering ? 'default' : 'pointer', fontSize: 14 }}
        >
